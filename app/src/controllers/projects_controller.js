...
export default class extends Controller {
  ...
  connect() {
    ...
    // Init next project page number for fetching paged results from API
    this.nextPage = 1;
  }

  load() {
    fetch(`https://1328d1c0.ngrok.io/api/v1/projects?per_page=12&page=${this.nextPage}`, {cache: "no-store"})
  	.then(response => {
      ...
      if (response.length > 0) {
        ...
        this.populateProjectCards(response);
      } else {
  ...
  loadOffline() {
    ...
    connect.onsuccess = function(event) {
      ...
      request.onsuccess = function(requestEvent) {
        that.populateProjectCards(requestEvent.target.result);
      }
    };
  }
  ...
  populateProjectCards(data) {
    ...
    document.getElementById("project-cards").appendChild(row_div);
  	...
  	for (var key in data) {
        ...
        col_div.setAttribute("href", `/tasks.html?project=${data[key].id}`);
        col_div.setAttribute("class", "project-link col-sm-4");
